...
The constructor for `TraceIDRatioBased` MUST accept ratios in the
range 0 through 1, inclusive.  When the input ratio is not exactly a
power of two greater than or equal to 2**-61, the Sampler MUST round
the ratio down to the nearest power of two that is greater than or
equal to 2**-61 or down to zero.
...
random bits limited to maximum value 61.  Here is example code to compute a
r-value:

```golang
// nextRValue returns an r-value variable.
func nextRValue() int {
        // The following shifts 63 random bits left by one and adds three 
		// 1-bits to ensure a maximum value of 61 from LeadingZeros64().
		rv := (uint64(rand.Int63()) << 1) | 0x7
		return bits.LeadingZeros64(rv)
}
```
